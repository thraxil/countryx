...
class Command(BaseCommand):
	...
	def get_or_create_group(self, groupname, section):
		try:
			...
			SectionGroupState.objects.create(group=group, state=start_state, date_updated=datetime.datetime.now())
	...
	def get_or_create_section(self, sectionName, term, year, first_turn):
		...
		dt = datetime.datetime.fromtimestamp(time.mktime(time.strptime(first_turn, time_format)))
		...
		try:
			...
			SectionTurnDates.objects.create(section=section, turn1=dt)
		return section
	...
	option_list = BaseCommand.option_list + (
        ...
        make_option('--first_turn', dest='first_turn', help='Date of the section first turn close'),
	...
	def handle(self, *app_labels, **options):
		args = 'Usage: python manage.py import_groups --csv csv file --section --term --year [--clean] --first_turn YYYY-MM-DD HH:MM'
		...
		section = self.get_or_create_section(section, term, year, first_turn)
