...
class LoginTestCases(TestCase):
    ...
    def test_index_not_logged_in(self):
        ...
        self.assertRedirects(response,
                             expected_url="/accounts/login/?next=/sim/",
                             status_code=302, target_status_code=200)

    def test_django_login(self):
        client = Client()

        # Issue a POST request for the login page that has the required
        # username,password supplied
        response = client.post('/accounts/login/',
                               {'username': 'testuser',
                                'password': 'test', "next": "/sim/"})

        self.assertRedirects(response, expected_url="/sim/",
                             status_code=302, target_status_code=200)

        # Now that the user is logged in, verify they are
    ...
    def test_django_faculty_login(self):
        ...
        response = client.post('/accounts/login/',
                               {'username': 'testfaculty',
                                'password': 'faculty', "next": "/sim/"})

        self.assertRedirects(response, expected_url="/sim/",
                             status_code=302, target_status_code=200)

        # Now that the user is logged in,
        # verify they are sent to the right place
        response = client.get('/sim/')
